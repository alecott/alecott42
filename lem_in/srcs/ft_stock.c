...
/*   Updated: 2018/02/25 12:09:55 by alecott          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_stock_tubes(t_ants *info, char *line)
{
	...
	while (ft_istubes(line))
	{
		...
		n++;
		info->tubes = ft_realloc(info->tubes, sizeof(char *) * n, sizeof(char *) * (n + 1));
		i++;
	}
	ft_strdel(&line);
	return (0);
...
int			ft_stock(t_ants *info)
{
	...
	while (get_next_line(0, &line) > 0 && ft_isroom(line))
	{
		...
		line = 0;
		n++;
		info->rooms = ft_realloc(info->rooms, sizeof(char *) * n, sizeof(char *) * (n + 1));
		i++;
	}
	info->rooms[i] = NULL;
	i = 0;
